...
class SmartManipulator(object):
    ...
    def switch_translate_manipulator(self):
        ...
        current_mode = self._utils.get_current_move_mode()
        self._utils.reset_rotate_mode(self._transform_modes[c.ROTATE][-1][c.INDEX])
        self._utils.reset_scale_mode(self._transform_modes[c.SCALE][-1][c.INDEX])
        ...
        for i, mode in enumerate(self._transform_modes[c.MOVE]):
            ...
            if not current_mode == self._transform_modes[c.MOVE][-1][c.INDEX]:
                if current_mode == self._transform_modes[c.MOVE][i][c.INDEX]:
                    self._utils.set_current_move_mode(self._transform_modes[c.MOVE][i + 1][c.INDEX])
                    info.show_info('W: {0}'.format(self._transform_modes[c.MOVE][i+1][c.NAME]))
                    cmd.undoInfo(stateWithoutFlush=True)
                    self._utils.fix_translate_marking_menu()
                    return

        self._utils.set_current_move_mode(self._transform_modes[c.MOVE][0][c.INDEX])
    ...
    def switch_rotate_manipulator(self):
        ...
        """
        cmd.undoInfo(stateWithoutFlush=False)
        current_mode = self._utils.get_current_rotate_mode()
        self._utils.reset_move_mode(self._transform_modes[c.MOVE][-1][c.INDEX])
        self._utils.reset_scale_mode(self._transform_modes[c.SCALE][-1][c.INDEX])
        ...
        for i, mode in enumerate(self._transform_modes[c.ROTATE]):
            ...
            if not current_mode == self._transform_modes[c.ROTATE][-1][c.INDEX]:
                if current_mode == self._transform_modes[c.ROTATE][i][c.INDEX]:
                    self._utils.set_current_rotate_mode(self._transform_modes[c.ROTATE][i + 1][c.INDEX])
                    info.show_info('E: {0}'.format(self._transform_modes[c.ROTATE][i+1][c.NAME]))
                    cmd.undoInfo(stateWithoutFlush=True)
                    self._utils.fix_rotate_marking_menu()
                    return

        self._utils.set_current_rotate_mode(self._transform_modes[c.ROTATE][0][c.INDEX])
    ...
    def switch_scale_manipulator(self):
        ...
        """
        cmd.undoInfo(stateWithoutFlush=False)
        current_mode = self._utils.get_current_scale_mode()
        self._utils.reset_move_mode(self._transform_modes[c.MOVE][-1][c.INDEX])
        self._utils.reset_rotate_mode(self._transform_modes[c.SCALE][-1][c.INDEX])
        ...
        for i, mode in enumerate(self._transform_modes[c.SCALE]):
            ...
            if not current_mode == self._transform_modes[c.SCALE][-1][c.INDEX]:
                if current_mode == self._transform_modes[c.SCALE][i][c.INDEX]:
                    self._utils.set_current_scale_mode(self._transform_modes[c.SCALE][i + 1][c.INDEX])
                    info.show_info('R: {0}'.format(self._transform_modes[c.SCALE][i+1][c.NAME]))
                    cmd.undoInfo(stateWithoutFlush=True)
                    self._utils.fix_scale_marking_menu()
                    return

        self._utils.set_current_scale_mode(self._transform_modes[c.SCALE][0][c.INDEX])
