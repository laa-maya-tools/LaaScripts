    ...
    if target == "keysSel" or target == "keysIndexSel":
        if animCurves:
            ...
            if getFrom == "graphEditor":
                for node in animCurves:
                    if target == "keysSel":         keysSel.append(
                        cmds.keyframe(node, selected=True, query=True, timeChange=True))
                    if target == "keysIndexSel":    keysSel.append(
        ...
        if animCurves:
            ...
            for n, node in enumerate(animCurves):
                ...
                for loopIndex in indexes:
                    keyTangents[n].append(
                        cmds.keyTangent(node, query=True, index=(loopIndex, loopIndex), inAngle=True, outAngle=True))
        ...
        if animCurves:
            ...
            for n, node in enumerate(animCurves):
                ...
                for loopIndex in indexes:
                    keyTangents[n].append(
                        cmds.keyTangent(node, query=True, index=(loopIndex, loopIndex), inTangentType=True,
                                        outTangentType=True))
                ...
                for n in range(100):  # find transform node (obj) and attribute name
                    ...
                    while type == "animBlendNodeEnum":  # skip anim layer nodes
                        ...
                        if cmds.objectType(node) == "animBlendNodeAdditiveRotation":
                            xyz = node[-1:]
                            node = cmds.listConnections("%s.output%s" % (node.split(".")[0], xyz), source=False,
                                                        destination=True, plugs=True, skipConversionNodes=True)
                            if node is None:
                                continue
                            else:
                                node = node[0]
                        else:
                            node = cmds.listConnections("%s.output" % node.split(".")[0], source=False,
                                                        destination=True, plugs=True, skipConversionNodes=True)
...
def getTimelineTime():
    timelineTime = cmds.currentTime(query=True);
    timelineTime = (timelineTime, timelineTime)
    return timelineTime
...
def isNodeRotate(node, xyz=None):
    ...
    if type == "doubleAngle":
        if xyz:
            if attr.find("rotate%s" % xyz.upper()) != -1 or (
                    attr.find("Merged_Layer_input") != -1 and attr.find(xyz.upper()) != -1):
                isRotate = True
        else:
            isRotate = True
...
def isNodeTranslate(node, xyz=None):
    ...
    if type == "doubleLinear":
        if xyz:
            if attr.find("translate%s" % xyz.upper()) != -1 or (
                    attr.find("Merged_Layer_input") != -1 and attr.find(xyz.upper()) != -1):
                isTranslate = True
        else:
            isTranslate = True
...
def channelBoxSel():
    ...
    if mObj and mAttr: channelsSel.extend(["%s.%s" % (loopObj, loopAttr) for loopObj in mObj for loopAttr in mAttr if
                                           cmds.objExists("%s.%s" % (loopObj, loopAttr))])
    if sObj and sAttr: channelsSel.extend(["%s.%s" % (loopObj, loopAttr) for loopObj in sObj for loopAttr in sAttr if
                                           cmds.objExists("%s.%s" % (loopObj, loopAttr))])
    if hObj and hAttr: channelsSel.extend(["%s.%s" % (loopObj, loopAttr) for loopObj in hObj for loopAttr in hAttr if
                                           cmds.objExists("%s.%s" % (loopObj, loopAttr))])
    if oObj and oAttr: channelsSel.extend(["%s.%s" % (loopObj, loopAttr) for loopObj in oObj for loopAttr in oAttr if
                                           cmds.objExists("%s.%s" % (loopObj, loopAttr))])
...
def getAllChannels(objs=None, changed=False, withAnimation=True):
    ...
    for loopObj in objs:
        ...
        allChannels.append(cmds.listAttr(loopObj, settable=True, keyable=True, locked=False, write=True, read=True,
                                         changedSinceFileOpen=isReference))
        ...
        if shapes:
            for loopShape in shapes:
                newChannel = cmds.listAttr(loopShape, userDefined=True, settable=True, keyable=True, locked=False,
    ...
    if curvesShown:
        ...
        for n, loopCurve in enumerate(curvesShown):
            ...
            if max(keyValues) != min(keyValues):
                cmds.selectionConnection('graphEditor1FromOutliner', edit=True,
                                         select="%s.%s" % (objsAttrs[0][n], objsAttrs[1][n]))
...
def getAnimData(animCurves=None, showProgress=None):
    ...
    for thisStep, loopCurve in enumerate(animCurves):
        ...
        if showProgress: startChrono = utilMod.chronoStart(startChrono, firstStep, thisStep, totalSteps, estimatedTime,
                                                           status)
...
def applyAnimData(animData, pasteInPlace=True, onlySelectedNodes=False, showProgress=None, status=None):
    if animData:
        ...
        if not onlySelectedNodes:
            ...
            if len(objects) > 0: objects = [loopObj for loopObj in objects if
                                            loopObj is not None and cmds.objExists(loopObj)]
            ...
            for n, key in enumerate(keys):
                ...
                if showProgress:
                    if cmds.progressBar(G.progBar, query=True, isCancelled=True):
                        ...
                        return
                    startChrono = utilMod.chronoStart(startChrono, firstStep, thisStep, totalSteps, estimatedTime,
                                                      status)
