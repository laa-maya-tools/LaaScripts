...
def cameraOrientMoveManip():
    ...
    with G.aToolsBar.createAToolsNode:
        constraint = cmds.aimConstraint(cameraNode, locator, name="tempCameraOrient_constraint", aimVector=[0, 0, 1],
                                        worldUpType="objectrotation", worldUpObject=cameraNode, maintainOffset=False)[0]
            ...
            if channelboxSelObjs:
                ...
                for n, loopObjAttr in enumerate(channelboxSelObjs):
                    ...
                    if not tangentType:  # if there is no key
                        ...
                        cmds.setKeyframe(loopObjAttr, time=time, insert=False, shape=False, inTangentType=inTangentType,
                    ...
                    cmds.setKeyframe(loopObjAttr, time=time, insert=insert, shape=False, inTangentType=inTangentType,
                                     outTangentType=outTangentType)
                ...
                tangentTypes = [cmds.keyTangent(obj, query=True, outTangentType=True, time=(prevKeys[n], prevKeys[n]))
...
def goToKey(which, type="key"):
    ...
    cmds.refresh(suspend=True)
    frame = cmds.findKeyframe(timeSlider=True, which=which) if type == "key" else cmds.currentTime(query=True) + (
        1 if which == "next" else -1)
...
def smartSnapKeys():
    ...
    for thisStep, loopCurve in enumerate(animCurves):
        ...
        stepKeys = [loopKey for nn, loopKey in enumerate(keyTimes[thisStep]) if
                    loopKey != round(loopKey) and loopKey in keysSel[thisStep] and keyTangentsType[thisStep][nn][
                        1] == "step"]
        linearKeys = [loopKey for nn, loopKey in enumerate(keyTimes[thisStep]) if
                      loopKey != round(loopKey) and loopKey in keysSel[thisStep] and keyTangentsType[thisStep][nn][
                          1] == "linear"]
        decimalKeys = [loopKey for nn, loopKey in enumerate(keyTimes[thisStep]) if
                       loopKey != round(loopKey) and loopKey in keysSel[
                           thisStep] and loopKey not in stepKeys + linearKeys]
