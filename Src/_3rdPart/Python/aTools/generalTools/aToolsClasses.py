...
class DeferredManager(object):
    ...
    def __init__(self):
        ...
        if G.deferredManager:
            self.queue = G.deferredManager.queue
        else:
            self.queue = {}
    ...
    def runQueue(self):
        ...
        if len(self.queue[priority]) > 0:
            ...
            try:
                ...
                print("aTools Deferred Manager Error#%s/%s: %s" % (priority, id, function))
...
class CallbackManager(object):
    ...
    def __init__(self):
        ...
        if G.callbackManager:
            self.queue = G.callbackManager.queue
        else:
            self.queue = {}
    ...
    def removeFromQueue(self, id):
        ...
        for loopQueue in self.queue[id]:
            ...
            if loopType == "scriptJob":
                self.removeScriptJob(loopJob, loopType, id)
            else:
                self.removeApiCallback(loopJob, loopType, id)
    ...
    def removeScriptJob(self, job, type, id):
        ...
        try:
            ...
            print(("aTools CallbackManager could not remove job %s/%s/%s" % (id, type, job)))
    ...
    def removeApiCallback(self, job, type, id):
        ...
        try:
            ...
            cmds.warning("aTools CallbackManager could not remove job %s/%s/%s" % (id, type, job))
