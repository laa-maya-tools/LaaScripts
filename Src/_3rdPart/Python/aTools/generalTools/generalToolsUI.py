...
from aTools.generalTools import hotkeys;
import importlib as imp

imp.reload(hotkeys)
from aTools.generalTools import tumbleOnObjects;

imp.reload(tumbleOnObjects)
from aTools.animTools import animationCrashRecovery;

imp.reload(animationCrashRecovery)
from aTools.animTools import framePlaybackRange;

imp.reload(framePlaybackRange)
from aTools.animTools import jumpToSelectedKey;

imp.reload(jumpToSelectedKey)
...
KEYSLIST = ["Up", "Down", "Left", "Right", "", "Page_Up", "Page_Down", "Home", "End", "Insert", "", "Return", "Space",
                                              ...
                                              "default": False
                                                  ...
                                                  "default": True
                                                      ...
                                                      "default": True
                                                          ...
                                                          "default": cmds.dollyCtx('dollyContext', query=True,
                                                                                   dollyTowardsCenter=True)
                                                          }, {"name": "cycleCheck",
...
class GeneralTools_Gui(uiMod.BaseSubUI):
    ...
    def createLayout(self):
        ...
        # selection
        cmds.iconTextButton("selectionCounterButton", style='textOnly', font="smallPlainLabelFont", label='0',
                            h=self.hb, annotation="Selected objects")
        ...
        # menu
        cmds.iconTextButton(style='iconOnly', w=self.wb, h=self.hb, image=uiMod.getImagePath("aTools"),
                            highlightImage=uiMod.getImagePath("aTools copy"), annotation="aTools Menu")
    ...
    def populateaToolsMenu(self, menu, button, *args):
        ...
        subMenu = cmds.menuItem("animBotMenu", subMenu=True, label='animBot - the new aTools', tearOff=True,
                                parent=menu)
        ...
        for loopPref in shortPrefs:
            name = loopPref["name"]
            cmds.menuItem('%sMenu' % name, label=utilMod.toTitle(name),
                          command=lambda x, name=name, *args: self.setPref(name), checkBox=self.getPref(name),
                          parent=menu)

        # ANIMATION CRASH RECOVERY
        animationCrashRecoveryPref = PREFS[4]
        cmds.menuItem("animationCrashRecoveryMenu", label='Animation Crash Recovery',
                      command=lambda *args: self.setPref(animationCrashRecoveryPref["name"]),
                      checkBox=self.getPref(animationCrashRecoveryPref["name"]), parent=menu)
    ...
    def setDefaultConfig(self):
        ...
        cmds.dollyCtx('dollyContext', edit=True, alternateContext=True, scale=1.0, localDolly=True,
        ...
        cmds.timeControl(G.playBackSliderPython, edit=True, showKeys="mainChannelBox", showKeysCombined=True,
    ...
    def uninstall(self, *args):
        ...
        confirm = cmds.confirmDialog(title='Confirm', message=message, button=['Yes', 'No'], defaultButton='No',
                                     cancelButton='No', dismissString='No')

        if confirm == 'Yes':

            from aTools.animTools.animBar import animBarUI;
            imp.reload(animBarUI)
    ...
    def about(self, warnUpdate=None, *args):
        ...
        # =========================================
        object = cmds.iconTextButton(label="Buy Me a Beer!", style="iconAndTextVertical", bgc=(.3, .3, .3), h=45, w=w,
                                     command=lambda *args: webbrowser.open_new_tab(DONATE_URL),
                                     image=uiMod.getImagePath("beer"), highlightImage=uiMod.getImagePath("beer copy"))
...
class Update(object):
    ...
    def hasUpdate(self):
        ...
        if utilMod.hasInternet(UPDATE_URL):
            try:
                response = urllib.request.urlopen(UPDATE_URL, timeout=60)
            except:
                pass
    ...
    def atoolsIsRetiring(self):
        ...
        object = cmds.button(label="1) Install animBot and have fun!", bgc=(.3, .3, .3), h=45, w=280,
                             command=installAnimBot)
        cmds.formLayout(form, edit=True, attachForm=[(object, 'top', pos), (object, 'left', 10)])
        pos += 45

        object = cmds.button(label="2) Watch the launch video.", bgc=(.3, .3, .3), h=45, w=280,
                             command=lambda *args: webbrowser.open_new_tab("https://youtu.be/DezLHqXrDao"))
        cmds.formLayout(form, edit=True, attachForm=[(object, 'top', pos), (object, 'left', 10)])
        pos += 45

        object = cmds.button(label="3) Join the community.", bgc=(.3, .3, .3), h=45, w=280,
                             command=lambda *args: webbrowser.open_new_tab(
                                 "https://www.facebook.com/groups/1589262684419439"))
        cmds.formLayout(form, edit=True, attachForm=[(object, 'top', pos), (object, 'left', 10)])
        pos += 65

        object = cmds.text(align="left",
                           label="The upgrade will be optional and although aTools\nwon't get feature updates, it will be available forever.\nPlease check the community for information about\nanimBot development progress.")
        cmds.formLayout(form, edit=True, attachForm=[(object, 'top', pos), (object, 'left', 10)])
        pos += 80
...
class SelectionCounter(object):
    ...
    def populateMenu(self, parent, *args):
        ...
        for loopSel in selection:
            cmds.menuItem('%sMenu' % loopSel, label=loopSel, parent=parent,
                          command=lambda x, loopSel=loopSel, *args: self.selectFromMenu(loopSel))
    ...
    def switch(self, onOff):
        ...
        if onOff:
            ...
            G.selectionCounterScriptJobs.append(
                cmds.scriptJob(runOnce=False, killWithScene=False, event=('SelectionChanged', self.update)))
    ...
    def switch(self, onOff):
        ...
        if onOff:
            G.autoSmartSnapKeysJobs.append(
                cmds.scriptJob(runOnce=False, killWithScene=False, event=('timeChanged', self.smartSnapKeys)))
...
class CommandsAndHotkeys(object):
    ...
    def openGui(self, *args):
        ...
        for loopColumn in range(totalColums):
            ...
            parent = cmds.rowColumnLayout(numberOfColumns=7,
                                          columnSpacing=([2, 5], [3, 3], [4, 3], [5, 1], [6, 5], [7, 5]),
            ...
            for loopIndex, loopCommand in enumerate(hotkeysDict[loopColumn]):
                ...
                cmds.button("command%s" % name, label=utilMod.toTitle(name), command=command, annotation=toolTip,
                            h=height, bgc=color, parent=parent)
                cmds.checkBox('ctl%s' % name, label='', value=ctl,
                              changeCommand=lambda x, name=name, *args: self.updateHotkeyCheck(name), h=height,
                              parent=parent)
                cmds.checkBox('alt%s' % name, label='', value=alt,
                              changeCommand=lambda x, name=name, *args: self.updateHotkeyCheck(name), h=height,
                              parent=parent)
                cmds.scrollField('key%s' % name, w=80, text=key,
                                 keyPressCommand=lambda x, name=name, *args: self.updateHotkeyCheck(name), h=height,
                                 parent=parent)
                cmds.button(label=" ", h=height, parent=parent)
                self.popSpecialHotkeys(name)
                cmds.button(label='>', command=lambda x, name=name, command=command, *args: self.setHotkey(
                    self.getHotkeyDict([name], [command])), h=height, parent=parent)
                cmds.text("query%s" % name, align="left", label=self.hotkeyCheck(key, ctl, alt), font="plainLabelFont",
                          h=height, parent=parent)
        ...
        cmds.button(label="Set All Hotkeys", command=lambda *args: self.setHotkey(self.getHotkeyDict(names, commands)),
                    parent=mainLayout)
    ...
    def popSpecialHotkeys(self, name):
        ...
        for loopKey in KEYSLIST:
            if loopKey == "":
                ...
                cmds.menuItem("menu%s" % loopKey, label=str(loopKey),
                              command=lambda x, name=name, loopKey=loopKey, *args: self.typeSpecialKey(name, loopKey))
    ...
    def setHotkey(self, hotkeyDict):
        ...
        confirm = cmds.confirmDialog(title='Confirm', message=message, button=['Yes', 'No'], defaultButton='Yes',
                                     cancelButton='No', dismissString='No')
...
def shelfButton(*args):
    ...
    cmds.shelfButton(parent=currentShelf, annotation='aTools ON/OFF', imageOverlayLabel="aTools", i='commandButton.xpm',
                     command=command)
...
def installAnimBot(*args):
    ...
    installFilePath = os.path.join(installFileFolder, "animBot Drag'n Drop Install.mel").replace("\\",
                                                                                                 "/")  # fix for windows
