...
from aTools.animTools.animBar.subUIs import tangents;
import importlib as imp

imp.reload(tangents)
from aTools.animTools.animBar.subUIs.specialTools_subUIs import mirror;
...
class KeyTransform_Gui(uiMod.BaseSubUI):
    ...
    def createLayout(self):
        ...
        # precision transform
        cmds.iconTextButton(style='iconAndTextVertical', w=self.wb, h=self.hb,
                            image=uiMod.getImagePath("keyTransform_-"),
                            highlightImage=uiMod.getImagePath("keyTransform_- copy"),
                            command=lambda *args: keyTransform.applyPrecise(keyTransform.getPrecisionValue() * -1),
                            annotation="Decrease precise transform")
        cmds.floatField("precisionNumber", minValue=0.01, precision=2, step=.05, value=0.5,
                        annotation="Set precise transform value\nRight click for pre-defined values")
        cmds.popupMenu()
        for loopValueList in valueList:
            if loopValueList == "":
                cmds.menuItem(divider=True)
            else:
                cmds.menuItem("menu%s" % loopValueList, label=str(loopValueList),
                              command=lambda loopValueList=loopValueList, *args: keyTransform.setPrecisionValue(
                                  loopValueList))
        cmds.iconTextButton(style='iconAndTextVertical', w=self.wb, h=self.hb,
                            image=uiMod.getImagePath("keyTransform_+"),
                            highlightImage=uiMod.getImagePath("keyTransform_+ copy"),
                            command=lambda *args: keyTransform.applyPrecise(keyTransform.getPrecisionValue()),
                            annotation="Increase precise transform")

        # reset
        cmds.iconTextButton(style='iconAndTextVertical', w=self.wb, h=self.hb,
                            image=uiMod.getImagePath("keyTransform_reset"),
                            highlightImage=uiMod.getImagePath("keyTransform_reset copy"),
                            command=keyTransform.resetValue,
        ...
        cmds.menuItem(divider=True)
        cmds.menuItem(label="Translate, Rotate and Scale",
                      command=lambda *args: keyTransform.resetValue(["Translate", "Rotate", "Scale"]))
        # key
        cmds.iconTextButton(style='iconAndTextVertical', w=self.wb, h=self.hb,
                            image=uiMod.getImagePath("keyTransform_keykey"),
                            highlightImage=uiMod.getImagePath("keyTransform_keykey copy"),
                            command=keyTransform.shareEachOtherKeys,
                            annotation="Share each other keys\nRight click for options")
        cmds.popupMenu()
        cmds.menuItem(label="All Keys", command=lambda *args: keyTransform.shareEachOtherKeys("all"))

        # nudge
        cmds.iconTextButton(style='iconAndTextVertical', w=self.wb, h=self.hb,
                            image=uiMod.getImagePath("keyTransform_nudge_left"),
                            highlightImage=uiMod.getImagePath("keyTransform_nudge_left copy"),
                            command=lambda *args: nudge.nudgeKey(-1),
                            annotation="Nudge key left\nRight click for options")
        nudge.popupMenu("left")
        cmds.floatField("nudgeEnterField", minValue=1, precision=0, step=1, value=10,
                        annotation="Set precise nudge value", visible=False, w=1)
        cmds.popupMenu()
        cmds.menuItem(label="Hide", command=lambda *args: nudge.toggleEnterField(False))
        cmds.iconTextButton(style='iconAndTextVertical', w=self.wb, h=self.hb,
                            image=uiMod.getImagePath("keyTransform_nudge_right"),
                            highlightImage=uiMod.getImagePath("keyTransform_nudge_right copy"),
                            command=lambda *args: nudge.nudgeKey(1),
                            annotation="Nudge key right\nRight click for options")
...
class KeyTransformSlider_Gui(object):
    ...
    def populateSlider(self, mode="default"):
        ...
        if mode == "default":
            textValue = cmds.text("keyTransformSliderText", label="100", visible=False)
        else:
            textValue = cmds.text("keyTransformSliderText_%s" % mode, label="100", visible=False)

        slider = cmds.floatSlider('keyTransformSlider%s' % mode, w=keySliderW - 1, min=0, max=2, value=1,
                                  dragCommand=lambda x, mode=mode, *args: self.applyKeyTransform(
                                      'keyTransformSlider%s' % mode, mode),
                                  changeCommand=lambda x, mode=mode, *args: self.releaseKeyTransform(
                                      'keyTransformSlider%s' % mode, mode))

        if mode == "default":
            modeButton = cmds.iconTextButton("keyTransformSliderButton", style='iconAndTextVertical', w=imageW, h=16,
                                             annotation="Key Transform Slider Mode")
            self.popUpModes()

            blendToFrameTxtA = cmds.iconTextButton("keyTransformSliderBlendToFrameA", font="smallPlainLabelFont",
                                                   style='iconAndTextCentered', w=45, label=labelA, align="left",
                                                   command=lambda *args: self.blendToFrameSet('A'),
                                                   ann=blendToFrameAnn);
            self.blendToFramePopUp('A')
            blendToFrameTxtB = cmds.iconTextButton("keyTransformSliderBlendToFrameB", font="smallPlainLabelFont",
                                                   style='iconAndTextCentered', w=45, label=labelB, align="right",
                                                   command=lambda *args: self.blendToFrameSet('B'),
                                                   ann=blendToFrameAnn);
            self.blendToFramePopUp('B')

        else:
            icon = self.getIcon(mode)
            modeButton = cmds.iconTextButton("keyTransformSliderButton_%s" % mode, style='iconAndTextVertical',
                                             w=imageW, h=16, image=uiMod.getImagePath("keyTransform_%s" % icon),
                                             highlightImage=uiMod.getImagePath("keyTransform_%s copy" % icon),
                                             command=lambda mode=mode, *args: self.setSliderMode(mode),
                                             annotation="Set this mode in the main toolbar")

            if mode == "blendToFrame":
                blendToFrameTxtA = cmds.iconTextButton("keyTransformSliderBlendToFrameToolbarA",
                                                       font="smallPlainLabelFont", style='iconAndTextCentered', w=45,
                                                       label=labelA, align="left",
                                                       command=lambda *args: self.blendToFrameSet('A'),
                                                       ann=blendToFrameAnn);
                self.blendToFramePopUp('A')
                blendToFrameTxtB = cmds.iconTextButton("keyTransformSliderBlendToFrameToolbarB",
                                                       font="smallPlainLabelFont", style='iconAndTextCentered', w=45,
                                                       label=labelB, align="right",
                                                       command=lambda *args: self.blendToFrameSet('B'),
                                                       ann=blendToFrameAnn);
                self.blendToFramePopUp('B')
        ...
        for n, loopValue in enumerate(preSetDict["values"]):
            ...
            b = cmds.iconTextButton(style='iconAndTextVertical', w=13, h=13,
                                    command=lambda value=value, mode=mode, slider=slider, *args: self.tickKeyTransform(
                                        slider, mode, (1. + (value * -1))), ann=int(value * 100),
                                    image=uiMod.getImagePath('keyTransform_dot_a'),
                                    highlightImage=uiMod.getImagePath('keyTransform_dot_a copy'))
            cmds.formLayout("keyTransformSliderFormLayout%s" % mode, edit=True,
                            attachPosition=[(b, 'top', 0, topPos), (b, 'left', 0, buttonPosition)])
            b = cmds.iconTextButton(style='iconAndTextVertical', w=13, h=13,
                                    command=lambda value=value, mode=mode, slider=slider, *args: self.tickKeyTransform(
                                        slider, mode, (1. + value)), ann=int(value * 100),
                                    image=uiMod.getImagePath('keyTransform_dot_a'),
                                    highlightImage=uiMod.getImagePath('keyTransform_dot_a copy'))
            cmds.formLayout("keyTransformSliderFormLayout%s" % mode, edit=True,
                            attachPosition=[(b, 'top', 0, topPos), (b, 'right', 0, 100 - buttonPosition)])
        ...
        if mode != "default":
            ...
            modButton = cmds.iconTextButton("keyTransformSliderModButton_%s" % mode, style='textOnly',
                                            font="smallObliqueLabelFont", label="                         ",
                                            align="center")
    ...
    def populateButtons(self):
        ...
        cmds.iconTextButton(style='iconOnly', h=25, w=25, image=uiMod.getImagePath("keyTransform_x"),
                            highlightImage=uiMod.getImagePath("keyTransform_x copy"),
                            command=self.toggleAllModesToolbar, annotation="Hide toolbar")
    ...
    def populateBlendToFrameHistory(self, menu, aB, *args):
        ...
        for loopItem in items:
            cmds.menuItem(label=loopItem,
                          command=lambda x, aB=aB, loopItem=loopItem, *args: self.blendToFrameSet(aB, loopItem),
                          parent=menu)
        ...
        if animCurves:
            ...
            if len(keyTimes) > 0:
                ...
                subMenu = cmds.menuItem(label="Select Key", subMenu=True, command=self.blendToFrameClearHistory,
                                        parent=menu)
                divider = None
                ...
                for loopItem in keyTimes:
                    ...
                    if not divider:
                        if loopItem == currFrame:
                            cmds.menuItem(divider=True, parent=subMenu)
                        elif loopItem > currFrame:
                            divider = cmds.menuItem(divider=True, parent=subMenu)
                    cmds.menuItem(label=loopItem,
                                  command=lambda x, aB=aB, loopItem=loopItem, *args: self.blendToFrameSet(aB, loopItem),
                                  parent=subMenu)
    ...
    def populateModes(self, menu, *args):
        ...
        for n, loopMode in enumerate(self.modesDict):
            ...
            cmds.menuItem(label=label, radioButton=radioSelected,
                          command=lambda x, mode=mode, *args: self.setSliderMode(mode), parent=menu)
    ...
    def populateModifiers(self, menu, mode, *args):
        ...
        cmds.menuItem(label="None", radioButton=radioSelected,
                      command=lambda x, mode=mode, mod=mod, *args: self.setModifier(mode, mod), parent=menu)
    ...
    def refreshModifiersButtons(self):
        ...
        for loopMode in self.modesDict:
            mode = loopMode["mode"]
            label = "(%s)" % utilMod.toTitle(self.getModeModifier(mode)).replace(" ", "+") if self.getModeModifier(
                mode) else "..."
    ...
    def delayIcon(self, mode):
        ...
        if mode == "default":
            ...
            cmds.iconTextButton("keyTransformSliderButton", edit=True,
                                image=uiMod.getImagePath("keyTransform_%s" % icon),
                                highlightImage=uiMod.getImagePath("keyTransform_%s copy" % icon))
            ...
            cmds.iconTextButton("keyTransformSliderButton_%s" % mode, edit=True,
                                image=uiMod.getImagePath("keyTransform_%s" % icon))
    ...
    def releaseKeyTransformDef(self, slider, mode, *args):
        ...
        if G.KT_flatKeys:
            ...
            G.KT_function(G.KT_gMode, G.KT_animCurves, G.KT_indexes, G.KT_keyValues, G.KT_keyTimes, G.KT_keysSel,
                          G.KT_keyTangentsY, G.KT_keyTangentsX, tValue, G.KT_pushClick)
    ...
    def applyKeyTransform(self, slider, mode, tValue=None, unselectObjects=True, *args):
        ...
        if mode == "default":
            cmds.text("keyTransformSliderText", edit=True, label=showValue)
        else:
            cmds.text("keyTransformSliderText_%s" % mode, edit=True, label=showValue)
        ...
        if not G.KT_pushClick:  # first time
            ...
            if mode == "default":
                cmds.text("keyTransformSliderText", edit=True, visible=True)
            else:
                cmds.text("keyTransformSliderText_%s" % mode, edit=True, visible=True)
        ...
        if G.KT_animCurves:
            G.KT_pushClick = True
            function = lambda *args: self.defApply(G.KT_function,
                                                   [G.KT_gMode, G.KT_animCurves, G.KT_indexes, G.KT_keyValues,
                                                    G.KT_keyTimes, G.KT_keysSel, G.KT_keyTangentsY, G.KT_keyTangentsX,
                                                    tValue, G.KT_pushClickDef])
    ...
    def setPushValues(self, mode, animCurves, indexes, keyValues, keyTimes, keysSel, keyTangentsY, keyTangentsX, tValue,
                      pushClick):
            ...
            for s, segment in enumerate(indexes[n]):
                ...
                for i, loopKeySel in enumerate(indexes[n][s]):
                    if 1 <= i <= len(indexes[n][s]) - 2:
                        ...
                        inTangentType = \
                        cmds.keyTangent(loopCurve, query=True, index=(index, index), inTangentType=True)[0]
                        outTangentType = \
                        cmds.keyTangent(loopCurve, query=True, index=(index, index), outTangentType=True)[0]
    ...
    def setNoiseValues(self, mode, animCurves, indexes, keyValues, keyTimes, keysSel, keyTangentsY, keyTangentsX,
                       tValue, pushClick):
        # set values
        for n, loopCurve in enumerate(animCurves):
            for s, segment in enumerate(indexes[n]):
                ...
                for i, loopKeySel in enumerate(indexes[n][s]):
                    ...
                    if 1 <= i <= len(indexes[n][s]) - 2:
                        ...
                        if index < len(keyValues[n]) - 4:
                            ...
                            if outTangentType == "fixed": cmds.keyTangent(loopCurve, index=(index, index),
                                                                          outAngle=angle)
                            ...
                            if inTangentType == "fixed": cmds.keyTangent(loopCurve, index=(index, index), iy=niy,
                                                                         ix=nix)
                            if outTangentType == "fixed": cmds.keyTangent(loopCurve, index=(index, index), oy=noy,
                                                                          ox=nox)

    def setBlendScaleValues(self, mode, animCurves, indexes, keyValues, keyTimes, keysSel, keyTangentsY, keyTangentsX,
                            tValue, pushClick):
        # set values

        if pushClick:  # avoid losing the middle point value
            if G.KT_lastTValue > 1 and tValue < 1:
                self.setBlendScaleValues(mode, animCurves, indexes, keyValues, keyTimes, keysSel, keyTangentsY,
                                         keyTangentsX, 1.0000001, pushClick)
            elif G.KT_lastTValue < 1 and tValue > 1:
                self.setBlendScaleValues(mode, animCurves, indexes, keyValues, keyTimes, keysSel, keyTangentsY,
                                         keyTangentsX, 1, pushClick)
        ...
        if mode == "blendToFrame":
            if tValue <= 1:
                if not self.blendToFrameCurrentValueA:
                    if tValue <= .97: cmds.warning(
                        "You need to select a frame first. Please go to some frame and hit the button A")
                    return
            else:
                if not self.blendToFrameCurrentValueB:
                    if tValue >= 1.03: cmds.warning(
                        "You need to select a frame first. Please go to some frame and hit the button B")
                    return
        ...
        if pushClick:  # second time on
            if G.KT_lastValue == 0:
                ...
                G.KT_scaleValue = 1 / G.KT_lastValue * p
        ...
        for n, loopCurve in enumerate(animCurves):
            ...
            if mode == "blendToFrame":
                if tValue <= 1:
                    f = self.blendToFrameCurrentValueA
                else:
                    f = self.blendToFrameCurrentValueB

                if loopCurve not in self.optimizedValues: self.optimizedValues[loopCurve] = {}
                if f not in self.optimizedValues[loopCurve]:
                    self.optimizedValues[loopCurve][f] = \
                    cmds.keyframe(loopCurve, query=True, eval=True, time=(f, f), valueChange=True)[0]
                ...
                for i, loopKeySel in enumerate(indexes[n][s]):
                    if 1 <= i <= len(indexes[n][s]) - 2:
                        ...
                        if mode == "blendToMirror":
                            ...
                            if not pushClick:
                                if isCenterCurve:
                                    pivot = v * mirrorInvertValue
                                else:
                                    pivot = cmds.keyframe(mCurve, query=True, eval=True, time=(f, f), valueChange=True)[
                                                0] * mirrorInvertValue
    ...
    def setEaseInOut(self, mode, animCurves, indexes, keyValues, keyTimes, keysSel, keyTangentsY, keyTangentsX, tValue,
                     pushClick):
        ...
        if animCurves:
            ...
            for n, loopCurve in enumerate(animCurves):
                ...
                for loopTime in time:
                    ...
                    tangType = cmds.keyTangent(loopCurve, query=True, time=loopTime, inTangentType=True,
                                               outTangentType=True)
                    if tangType[1] != "step":
...
class Nudge(object):
    ...
    def populateMenu(self, leftRight, *args):
        ...
        for loopValue in values:
            ...
            cmds.menuItem(label=abs(loopValue), command=lambda x, loopValue=loopValue, *args: self.nudgeKey(loopValue),
                          parent=menu)

        cmds.menuItem(label="Custom", command=lambda *args: self.toggleEnterField(True), parent=menu)
        cmds.menuItem(divider=True, parent=menu)
        cmds.menuItem(label="Nudge Everything in the Scene", checkBox=self.applyToEverything,
                      command=lambda *args: self.toggleEnterField(True, args), parent=menu)
