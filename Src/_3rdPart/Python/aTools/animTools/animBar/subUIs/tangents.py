...
class Tangents_Gui(uiMod.BaseSubUI):
    ...
    def createLayout(self):
        ...
        for loopButton in buttons:
            cmds.iconTextButton(style='iconAndTextVertical', image=uiMod.getImagePath("tangents_%s" % loopButton),
                                highlightImage=uiMod.getImagePath("tangents_%s copy" % loopButton), w=self.wb,
                                h=self.hb, command=lambda loopButton=loopButton, *args: tangents.setTangent(loopButton),
...
class Tangents(object):
    ...
    def popupMenu(self, button, *args):
        menu = cmds.popupMenu()
        cmds.popupMenu(menu, edit=True, postMenuCommand=lambda *args: self.populateMenu(menu, button),
    ...
    def populateMenu(self, menu, button, *args):
        ...
        if button != "step":
            ...
            cmds.menuItem(label='First Frame', command=lambda *args: self.setTangent(button, 'out', 'first'),
                          parent=menu)
            cmds.menuItem(label='Last Frame', command=lambda *args: self.setTangent(button, 'in', 'last'), parent=menu)
            cmds.menuItem(label='Both Ends', command=lambda *args: self.setTangent(button, 'inOut', 'both'),
                          parent=menu)
        ...
        if self.isDefaultTangent(tangentType):  # default maya tangents
                ...
                if keysIndexSel != None and keyTimes[n] != None and keysIndexSel[n] != None and len(keyTimes[n]) >= 2:
                    ...
                    for i in keysIndexSel[n]:
                        tangentArray[n].append(
                            cmds.keyTangent(aCurve, query=True, index=(i, i), inTangentType=True, outTangentType=True,
                                            inAngle=True, outAngle=True))
                ...
                if keysIndexSel != None and keyTimes[n] != None and keysIndexSel[n] != None and len(keyTimes[n]) >= 2:
                    ...
                    for nn, i in enumerate(keysIndexSel[n]):
                        ...
                        if tangentInOut == "in":
                            ...
                            cmds.keyTangent(aCurve, edit=True, index=(i, i), outTangentType=tangent[3],
                                            outAngle=tangent[1])
                            cmds.keyTangent(aCurve, edit=True, index=(i, i), lock=True)

                        elif tangentInOut == "out":
                            cmds.keyTangent(aCurve, edit=True, index=(i, i), lock=False)
                            cmds.keyTangent(aCurve, edit=True, index=(i, i), inTangentType=tangent[2],
                                            inAngle=tangent[0])
                            cmds.keyTangent(aCurve, edit=True, index=(i, i), lock=True)
            ...
            if tangentType == "flow":
                ...
                for n, aCurve in enumerate(animCurves):
                    ...
                    if first and last:
                        self.bounceEnds([aCurve], "bounce", getFrom, tangentInOut, [keyTimes[n]], [keyIndexTimes[n]],
                                        "both")
                    elif first:
                        self.bounceEnds([aCurve], "bounce", getFrom, tangentInOut, [keyTimes[n]], [keyIndexTimes[n]],
                                        "first")
        ...
        for loopFunction in passes:
            ...
            for n, aCurve in enumerate(animCurves):
                ...
                if keysIndexSel != None and keyTimes[n] != None and keysIndexSel[n] != None and len(keyTimes[n]) >= 2:
                    ...
                    if weighted: cmds.keyTangent(aCurve, edit=True,
                                                 weightedTangents=False)  # weight to balance in and out tangents
                    ...
                    for loopIndex in keysIndexSel[n]:
                        ...
                        applied = loopFunction(aCurve, keyValues[n], loopIndex, tangentType, curTangType,
                                               keysIndexSel[n], keyTimes[n])
        ...
        if animCurves:
            ...
            if targetKeys == "all":  # apply for all keys
                ...
                if getFrom == "timeline":
                    ...
                    if not self.isDefaultTangent(tangentType): index = animMod.getTarget("keysIndexSel", animCurves,
                                                                                         getFrom)
                    ...
                    if self.isDefaultTangent(tangentType):  # if the tangent types are default maya types
                        ...
                        for thisStep, aCurve in enumerate(animCurves):
                            if cmds.progressBar(G.progBar, query=True, isCancelled=True):
                                ...
                                break
                            startChrono = utilMod.chronoStart(startChrono, firstStep, thisStep, totalSteps,
    ...
    def tangType(self, keyVal, keyTimes, index):
        ...
        if keyLocation == "first":
            ...
            if keyValTmp[index] == keyValTmp[index + 1] == keyValTmp[index + 2] or keyValTmp[index] == keyValTmp[
                index - 1] == keyValTmp[index - 2] or keyValTmp[index] == keyValTmp[index + 1] == keyValTmp[index - 1]:
                return "Zero"
    ...
    def fixTangentOvershoot(self, aCurve, keyVal, index, tangentType, curTangType, keysIndexSelN, *args):
        ...
        # get in values
        iy = cmds.keyTangent(aCurve, query=True, index=(index, index), iy=True)[
                 0] / 3 * power  # in tangent handle y position
        oy = cmds.keyTangent(aCurve, query=True, index=(index, index), oy=True)[
                 0] / 3 * power  # out tangent handle y position
        ...
        if (difPrev < 0 and iy < difPrev) or (difPrev > 0 and iy > difPrev):
            ...
            if index - 1 in keysIndexSelN:
                cmds.keyTangent(aCurve, edit=True, index=(index - 1, index - 1), inTangentType="auto",
                                outTangentType="auto")
    ...
    def fixTangentOpposite(self, aCurve, keyVal, index, tangentType, curTangType, keysIndexSelN, *args):
        ...
        valIn = nextVal - \
                cmds.keyframe(aCurve, query=True, eval=True, time=(nextTime - ix / .125, nextTime - ix / .125),
                              valueChange=True)[0]
        valOut = cmds.keyframe(aCurve, query=True, eval=True, time=(currTime + ox / .125, currTime + ox / .125),
                               valueChange=True)[0] - currVal
        ...
        if (difIn > 0 and difOut > 0) or (difIn < 0 and difOut < 0):
            ...
            for x in range(5):
                ...
                ox = cmds.keyTangent(aCurve, query=True, index=(index, index), ox=True)[
                    0]  # out tangent handle x position
                oy = cmds.keyTangent(aCurve, query=True, index=(index, index), oy=True)[
                    0]  # out tangent handle y position
                ...
                if inOut == "in":
                    ...
                    if index + 1 in keysIndexSelN:
                        ...
                        cmds.keyTangent(aCurve, edit=True, index=(index + 1, index + 1), iy=newY * 3, oy=newY * 3,
                                        ox=ix)
    ...
    def flowTangent(self, aCurve, keyVal, index, tangentType, curTangType, *args):
        ...
        # curve position at handle
        valIn = cmds.keyframe(aCurve, query=True, eval=True, time=(currTime - ix / .125, currTime - ix / .125),
                              valueChange=True)[0]
        valOut = cmds.keyframe(aCurve, query=True, eval=True, time=(currTime + ox / .125, currTime + ox / .125),
                               valueChange=True)[0]
