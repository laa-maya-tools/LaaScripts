...
class Align(object):
    ...
    def align(self, sourceObjs, targetObj, frames=None, translate=True, rotate=True, showProgress=False,
        ...
        for thisStep, loopSourceObj in enumerate(sourceObjs):
            ...
            if showProgress: startChrono = utilMod.chronoStart(startChrono, firstStep, thisStep, totalSteps,
            ...
            for loopMode in modes:
                ...
                with G.aToolsBar.createAToolsNode: constraints.append(
                    contrainFn(targetObj, loopSourceObj, skip=skip)[0])

                setValues[-1]["modes"].append(mode)
                setValues[-1]["values"].append(
                    [cmds.getAttr("%s.%s" % (loopSourceObj, mode), time=loopKey)[0] for loopKey in frames])
            ...
            for nn, loopSourceObj in enumerate(sourceObjs):
                ...
                if n == len(frames) - 1 and rotate:
                    animCurves = utilMod.mergeLists(
                        [cmds.keyframe(loopSourceObj, query=True, name=True) for loopSourceObj in sourceObjs])
