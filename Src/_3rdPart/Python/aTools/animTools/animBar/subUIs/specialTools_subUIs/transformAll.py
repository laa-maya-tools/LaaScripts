...
class TransformAll(object):
    ...
    def blinkingButton(self, onOff):
        ...
        if onOff:
            G.aToolsBar.timeoutInterval.setInterval(self.toggleButtonActive, .3, id="TA_blinking")
        else:
    ...
    def setButtonImg(self, onOff):
        if onOff:
            cmds.iconTextButton("transformAllBtn", edit=True,
    ...
    def setMode(self, onOff):
        ...
        if onOff:
            ...
            G.transformAllScriptJobs.append(cmds.scriptJob(runOnce=False, killWithScene=False,
                                                           event=('SelectionChanged', self.updateCurrentValues)))
            ...
            for loopCurve in animCurves:
                ...
                if time:
                    ...
                    result["keyValues"].append(
                        cmds.keyframe(loopCurve, query=True, time=(time[0], time[-1]), valueChange=True))
                else:
    ...
    def setValues(self, animCurves):
        ...
        for n, loopCurve in enumerate(animCurves):
            ...
            if self.blendRangeMode:
                ...
                pivotA = \
                cmds.keyframe(loopCurve, query=True, eval=True, time=(self.range[0], self.range[0]), valueChange=True)[
                    0]
                pivotB = \
                cmds.keyframe(loopCurve, query=True, eval=True, time=(self.range[1], self.range[1]), valueChange=True)[
                    0]
        ...
        if self.blendRangeMode:
            for n, loopCurve in enumerate(animCurves):
                ...
                cmds.scaleKey(loopCurve, time=(self.range[0] + timeOffsetA, time[1]), valuePivot=pivotA,
                              valueScale=offsetPercentA)
                cmds.scaleKey(loopCurve, time=(time[1] + .01, self.range[1] - timeOffsetB), valuePivot=pivotB,
            ...
            for n, loopCurve in enumerate(animCurves):
                if offsetValues[n] != 0:
                    ...
                    if self.range == "All Keys":
                        ...
                        cmds.keyframe(loopCurve, edit=True, time=(self.range[0], self.range[1]),
                                      valueChange=offsetValues[n], relative=True)
    ...
    def warn(self):
        ...
        cmds.warning("Transform All %sis ON. Please remember to turn it OFF when you are done. Acting on range: %s" % (
        blendTxt, self.range))
    ...
    def setRange(self):
        ...
        if self.range[1] - self.range[0] <= 1:  # if only one key selected
            if self.blendRangeMode:
                self.range = [cmds.playbackOptions(query=True, minTime=True),
