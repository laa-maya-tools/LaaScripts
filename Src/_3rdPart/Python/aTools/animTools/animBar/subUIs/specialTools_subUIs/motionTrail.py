...
class MotionTrail(object):
    ...
    def populateMenu(self, menu, *args):
        ...
        if cmds.objExists(self.mainGroupNode):
            for loopMotionTrail in list(self.nodeInfo.keys()):
                ...
                cmds.menuItem(label="Grab Offset Ctrl (%s)" % ctrl, parent=menu,
                              command=lambda x, loopMotionTrail=loopMotionTrail, *args: self.grabOffsetCtrl(
                                  loopMotionTrail))
        ...
        for loopPref in self.prePosRanges:
            ...
            cmds.menuItem(label="%s" % loopPref, radioButton=radioSelected,
                          command=lambda x, loopPref=loopPref, *args: self.savePrePosRange(loopPref), parent=subMenu)
        ...
        for loopSize in self.customSizes:
            ...
            cmds.menuItem(label=utilMod.toTitle(sizeName), radioButton=radioSelected,
                          command=lambda x, sizeName=sizeName, *args: self.savePrefSize(sizeName), parent=subMenu)
        ...
        for loopPref in self.prefKeys:
            ...
            cmds.menuItem(label="Show %s Keys" % utilMod.toTitle(loopPref), radioButton=radioSelected,
                          command=lambda x, loopPref=loopPref, *args: self.savePrefKey(loopPref), parent=subMenu)

        # Camera Relative
        cmds.menuItem(divider=True, parent=subMenu)
        cmds.menuItem(label="Camera Relative", checkBox=self.cameraRelativeMode, command=self.setCameraRelativeMode,
                      parent=subMenu)
    ...
    def setMode(self, onOff):
        ...
        if onOff:
            ...
            cmds.iconTextButton(self.toolBarButton, edit=True,
                                image=uiMod.getImagePath("specialTools_motion_trail_active"),
                                highlightImage=uiMod.getImagePath("specialTools_motion_trail_active"))
            ...
            cmds.iconTextButton(self.toolBarButton, edit=True, image=uiMod.getImagePath("specialTools_motion_trail"),
                                highlightImage=uiMod.getImagePath("specialTools_motion_trail copy"))
    ...
    def addScriptJobs(self):
        ...
        G.callbackManager.sendToQueue(cmds.scriptJob(runOnce=False, killWithScene=False,
                                                     event=('SelectionChanged', self.callbackSelectionChange)),
                                      "scriptJob", "MT_scriptJobs")
        G.callbackManager.sendToQueue(
            cmds.scriptJob(runOnce=False, killWithScene=False, event=('timeChanged', self.callbackTimeChanged)),
            "scriptJob", "MT_scriptJobs")
    ...
    def addMessages(self):
        ...
        G.callbackManager.sendToQueue(OpenMayaAnim.MAnimMessage.addAnimCurveEditedCallback(self.callbackCurveEdited),
                                      "OpenMayaAnim.MAnimMessage", "MT_anim")

        for loopViewport in viewports:
            G.callbackManager.sendToQueue(
                OpenMayaUI.MUiMessage.add3dViewPostRenderMsgCallback(loopViewport, self.drawViewports),
                "OpenMayaUI.MUiMessage", "MT_view")
    ...
    def addNodeMessages(self):
        ...
        for loopSel in sel:
            ...
            G.callbackManager.sendToQueue(
                OpenMaya.MNodeMessage.addAttributeChangedCallback(node, self.callbackAttrEdited, None),
                "OpenMaya.MNodeMessage", "MT_node")
    ...
    def updateCache(self, lightningMode=False, progressInfo=None, *args):
        ...
        if lightningMode:
            ...
            G.deferredManager.sendToQueue(lambda *args: self.updateCache(lightningMode), self.queueOrder,
                                          "motionTrailUpdate")
    ...
    def createMotionTrailNode(self, masterNode):
        ...
        with G.aToolsBar.createAToolsNode:
            ...
            motionTrailTransform = cmds.rename(cmds.listRelatives(motionTrailShape, parent=True)[0],
                                               "%s_motionTrailTransform" % masterNode)
    ...
    def createMotionTrail(self, ctrls):
        ...
        for loopCtrl in ctrls:
            ...
            self.nodeInfo[ctrlName] = {"ctrlNode": loopCtrl, "offsetNode": offsetNode, "groupNode": groupNode,
                                       "motionTrailTransform": motionTrailTransform, "motionTrailAttr": motionTrailAttr,
                                       "motionTrailNode": motionTrailNode, "lineColor": next(self.lineColorCycle)}
            ...
            if ctrlName in self.offsetCtrlsPosition:
                ...
                if offsetNodePosition != (0, 0, 0): cmds.setAttr("%s.translate" % offsetNode, offsetNodePosition[0],
                                                                 offsetNodePosition[1], offsetNodePosition[2])
    ...
    def updateCameraMatrices(self):
        ...
        for loopCamera in cameraNodes:
            ...
            for loopFrame in self.sortedRange:
                ...
                self.cameraInfo[loopCamera][loopFrame]["pointMatrix"] = cmds.getAttr("%s.wm" % loopCamera,
                                                                                     time=loopFrame)
    ...
    def getPointsArray(self, motionTrail, currFrame, camera=None):
        ...
        range = [self.firstFrame, self.lastFrame] if self.prePosRange == 0 else [currFrame - self.prePosRange,
                                                                                 currFrame + self.prePosRange]
    ...
    def getIndexMap(self, motionTrail, keysTimes, currFrame, pointArray):
        ...
        indexMap = {"curr_frame": [], "key_before": [], "frame_before": [], "key_after": [], "frame_after": [],
                    "dirty": [], "ommited": []}
        indexOffset = self.firstFrame if self.prePosRange == 0 else currFrame - self.prePosRange if (
                                                                                                                currFrame - self.prePosRange) > self.firstFrame else self.firstFrame
        ...
        for loopIndex in range(len(pointArray)):
            ...
            if self.nodeInfo[motionTrail][frame]["status"] == "dirty":
                if self.blockingMode and frame not in keysTimes:
                    indexMap["ommited"].append(loopIndex)
                else:
                    indexMap["dirty"].append(loopIndex)
            elif frame == currFrame:
                indexMap["curr_frame"].append(loopIndex)
            elif frame < currFrame:
                if frame in keysTimes:
                    indexMap["key_before"].append(loopIndex)
                else:
                    indexMap["frame_before"].append(loopIndex)
            elif frame > currFrame:
                if frame in keysTimes:
                    indexMap["key_after"].append(loopIndex)
                else:
                    indexMap["frame_after"].append(loopIndex)
    ...
    def drawViewports(self, *args):
        ...
        for loopViewport in utilMod.getAllViewports():
            ...
            for loopMotionTrail in list(self.nodeInfo.keys()):
                ...
                viewportCamera = cmds.modelEditor(loopViewport, query=True,
                                                  camera=True) if self.cameraRelativeMode else None
                camera = viewportCamera if viewportCamera in list(
                    self.cameraInfo.keys()) and self.cameraRelativeMode else None
