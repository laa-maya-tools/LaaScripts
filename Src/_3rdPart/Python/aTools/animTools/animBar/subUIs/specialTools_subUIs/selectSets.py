...
class SelectSets(object):
    ...
    def popupMenu(self, *args):
        ...
        for loopColor in self.colors:
            ...
            cmds.menuItem('colorMenu_%s' % colorName, label=utilMod.toTitle(colorName), checkBox=False,
                          command=lambda x, colorName=colorName, *args: self.switchColor(colorName))
        ...
        for loopColor in self.colors:
            ...
            cmds.menuItem(label=utilMod.toTitle(colorName),
                          command=lambda x, colorName=colorName, *args: self.isolateColor(colorName))
    ...
    def toggleToolbar(self, forceOff=None):
        ...
        if cmds.iconTextButton("selectSetsBtn", query=True, exists=True):
            if visible:
                cmds.iconTextButton("selectSetsBtn", edit=True,
                                    image=uiMod.getImagePath("specialTools_select_sets_active"),
                                    highlightImage=uiMod.getImagePath("specialTools_select_sets_active"))
            else:
                cmds.iconTextButton("selectSetsBtn", edit=True, image=uiMod.getImagePath("specialTools_select_sets"),
                                    highlightImage=uiMod.getImagePath("specialTools_select_sets copy"))
    ...
    def createSelSetIfInexistent(self, selSet, nodes):
        if not cmds.objExists(selSet):
            ...
            if len(existentNodes) > 0:
                return cmds.sets(existentNodes, name=selSet, text="gCharacterSet")
            else:
                return
    ...
    def addScriptJobs(self):
        ...
        G.selectSetsScriptJobs.append(
            cmds.scriptJob(runOnce=True, killWithScene=False, event=('NewSceneOpened', self.refreshToolBar)))
        G.selectSetsScriptJobs.append(cmds.scriptJob(runOnce=False, killWithScene=False,
                                                     event=('SelectionChanged', self.highlightSelectedButtons)))

    def addSceneMessages(self):

        self.removeMessages()
        # SCENE MESSAGES
        G.SS_messages["scene"].append(
            OpenMaya.MSceneMessage.addCallback(OpenMaya.MSceneMessage.kBeforeSave, self.beforeSave))
        # G.SS_messages["scene"].append(OpenMaya.MSceneMessage.addCallback( OpenMaya.MSceneMessage.kAfterSave, self.afterSave))
        G.SS_messages["scene"].append(
            OpenMaya.MSceneMessage.addCallback(OpenMaya.MSceneMessage.kAfterOpen, self.refreshToolBar))
    ...
    def highlightColorSelection(self):
        ...
        for loopColor in self.colors:
            ...
            if loopColorName == G.SS_lastColorUsed:
                for loopField in fields: cmds.iconTextButton("colorButton%s%s" % (loopField, loopColorName), edit=True,
    ...
    def selSetButtonPopUp(self, selSet):
        menu = cmds.popupMenu()
        cmds.popupMenu(menu, edit=True, postMenuCommand=lambda *args: self.populateSelSetButtonsMenu(menu, selSet),
    ...
    def populateSelSetButtonsMenu(self, menu, selSet, *args):
        ...
        for loopColor in self.colors:
            ...
            cmds.menuItem(label=utilMod.toTitle(loopColorName), radioButton=radioSelected,
                          command=lambda x, selSet=selSet, loopColorName=loopColorName,
                                         *args: self.renameSelectSetColor([selSet], loopColorName), parent=menu)
        ...
        cmds.menuItem(label='Delete All %s' % utilMod.toTitle(colorName),
                      command=lambda *args: self.deleteAllColorSet(colorName), parent=menu)
        cmds.menuItem(divider=True, parent=menu)

        duplicateToOtherMenu = cmds.menuItem(subMenu=True, label='Duplicate To Other Character', parent=menu,
                                             postMenuCommandOnce=True)
        cmds.menuItem(duplicateToOtherMenu, edit=True,
                      postMenuCommand=lambda *args: self.populateDuplicateToOtherMenu(duplicateToOtherMenu, selSet))
        cmds.setParent('..', menu=True)

        cmds.menuItem(label='Show Only %s' % utilMod.toTitle(colorName),
                      command=lambda *args: self.isolateColor(colorName), parent=menu)
        cmds.menuItem(label='Hide %s' % utilMod.toTitle(colorName), command=lambda *args: self.hideColor(colorName),
                      parent=menu)
    ...
    def sortSelSetButtons(self, fromSelSet=None, renameSet=None):
        ...
        if not fromSelSet:
            ...
            index = self.aToolsSets.index(fromSelSet)
        ...
        for loopSet in self.aToolsSets[index:]:
            ...
            if loopSet in self.selSetButtonWidth: cmds.iconTextButton('aToolsSetBtn_%s' % loopSet, edit=True,
                                                                      w=self.selSetButtonWidth[loopSet])
    ...
    def testRepeated(self, sel, warn=True):
        ...
        for loopSet in self.aToolsSets:
            ...
            if cmds.sets(sel, isMember=loopSet) and len(sel) == cmds.sets(loopSet, query=True, size=True):
                ...
                if warn: cmds.warning("The selection is already a set: (%s) %s" % (
                color, loopSet[len(self.defaultSetName) + len(color) + 1:]))
                return True
    ...
    def getaToolsSets(self):
        ...
        allSelSets = [loopSet for loopSet in cmds.ls(sets=True, exactType="objectSet") if
                      cmds.sets(loopSet, query=True, text=True) == "gCharacterSet"]
        aToolsSets = []
            ...
            if self.defaultSetName in loopSet:
                if cmds.objExists(loopSet):
                    ...
                    for loopColor in self.colors:
                        ...
                        if self.defaultSetName in loopSet and colorName == loopColorName and cmds.sets(loopSet,
                                                                                                       query=True,
                                                                                                       text=True) == "gCharacterSet":
    ...
    def getSelectedSets(self):
        return [loopSet for loopSet in self.aToolsSets if
                cmds.iconTextButton('aToolsSetBtn_%s' % loopSet, query=True, exists=True) and cmds.iconTextButton(
                    'aToolsSetBtn_%s' % loopSet, query=True, style=True) != 'textOnly']
    ...
    def highlightSelectedButtons(self, onlySelSet=None):
        ...
        if len(CurrSel) > 0:
            ...
            for loopSet in aToolsSets:
                ...
                if nodes == CurrSel:
                    cmds.iconTextButton('aToolsSetBtn_%s' % loopSet, edit=True, style='iconAndTextVertical',
                                        image=bigDot, highlightImage=bigDot)
                elif set(CurrSel).issuperset(set(nodes)):
                    cmds.iconTextButton('aToolsSetBtn_%s' % loopSet, edit=True, style='iconAndTextVertical',
                                        image=smallDot, highlightImage=smallDot)
                else:
    ...
    def togleBlinkButton(self, button, color):
        ...
        if white:
            ...
            self.setButtonColor(button, (1, 1, 1))
    ...
    def blinkButton(self, selSet, xTimes):
        ...
        G.aToolsBar.timeoutInterval.setTimeout(
            (lambda *args: self.togleBlinkButton('aToolsSetBtn_%s' % selSet, colorValue)), sec=.05, xTimes=(xTimes * 2))
    ...
    def populateDuplicateToOtherMenu(self, menu, selSet, *args):
        ...
        cmds.menuItem(newMenu, edit=True,
                      postMenuCommand=lambda *args: self.populateDuplicateButtonMenu(newMenu, [selSet], colorName))
        cmds.menuItem(newMenuAllColors, edit=True,
                      postMenuCommand=lambda *args: self.populateDuplicateButtonMenu(newMenuAllColors, allColorSets,
                                                                                     colorName))
    ...
    def populateDuplicateButtonMenu(self, menu, selSet, colorName):
        ...
        for loopNameSpace in outputNameSpaces:
            ...
            for loopColor in self.colors:
                ...
                if colorName != loopColorName:
                    cmds.menuItem(label=utilMod.toTitle(loopColorName), parent=newMenu,
                                  command=lambda x, loopNameSpace=loopNameSpace, loopColorName=loopColorName,
                                                 *args: self.duplicateSet(selSet, loopNameSpace, loopColorName))
    ...
    def duplicateSet(self, selSets, outputNameSpace, newColor):
        ...
        for inputNameSpace in inputNameSpaces:
            ...
            selSetsData = eval(
                selSetsStr.replace("%s%s" % (inputNameSpace, separator), "%s%s" % (outputNameSpace, separator)))
