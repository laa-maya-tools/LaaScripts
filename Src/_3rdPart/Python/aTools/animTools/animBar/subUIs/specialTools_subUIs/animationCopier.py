...
class AnimationCopier(object):
    ...
    def pasteAnimation(self, animData=None, pasteInPlace=True, onlySelectedNodes=None, *args):
        ...
        if not onlySelectedNodes:   onlySelectedNodes = cmds.menuItem("onlySelectedNodesMenu", query=True,
                                                                      checkBox=True)
        ...
        for loopNameSpace in inputNameSpaces:
            ...
            if outputNameSpaces:
                ...
                for loopOutput in outputNameSpaces:
                    cmds.menuItem("menu%s" % loopOutput, label=str(loopOutput),
                                  command=lambda x, loopOutput=loopOutput, nameSpace=nameSpace,
                                                 *args: self.setOutputValue(loopOutput, nameSpace))

        cmds.setParent('..')

        cmds.button(label="Paste Animation in Place",
                    command=lambda *args: self.remapAndPasteAnimation(animData, inputNameSpaces, pasteInPlace=True))
        cmds.button(label="Paste Original Animation",
                    command=lambda *args: self.remapAndPasteAnimation(animData, inputNameSpaces, pasteInPlace=False))
