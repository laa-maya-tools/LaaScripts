...
class SelectionManager(object):
    ...
    @staticmethod
    def get_ctrls_list(ctrls_node, ctrls_list_attr):
        main_ctrls = []
        second_ctrls = []
        sets = SceneData.load_scene_data(ctrls_node, ctrls_list_attr).split(c.SEPARATOR.MAIN)
        for set in sets:
            ctrls = set.split(c.SEPARATOR.SECOND)
            if len(ctrls) > 1:
                for ctrl in ctrls:
                    second_ctrls.append(ctrl)
                main_ctrls.append(second_ctrls)
            else:
                main_ctrls.append([set])

        return main_ctrls

    def next_upperbody_ctrl(self):
        upperbody_ctrls = SelectionManager.get_ctrls_list(c.NODES.UPPERBODY_NODE, c.NODES.CTRLS_LIST_ATTR)
        # char_nsp = CharacterInfo.get_active_char_info()
        #
