...
class MicroTransform(object):
    ...
    def __init__(self):
        ...
        self.attributes = ['translate', 'translateX', 'translateY', 'translateZ', 'rotate', 'rotateX', 'rotateY',
    ...
    def blinkingButton(self, onOff):
        ...
        if onOff:
            G.aToolsBar.timeoutInterval.setInterval(self.toggleButtonActive, .3, id="MT_blinking")
        else:
    ...
    def setButtonImg(self, onOff):
        if onOff:
            cmds.iconTextButton("microTransformBtn", edit=True,
                                image=uiMod.getImagePath("specialTools_micro_transform_active"),
                                highlightImage=uiMod.getImagePath("specialTools_micro_transform_active"))
        else:
            cmds.iconTextButton("microTransformBtn", edit=True,
                                image=uiMod.getImagePath("specialTools_micro_transform"),
                                highlightImage=uiMod.getImagePath("specialTools_micro_transform copy"))
    ...
    def setMode(self, onOff):
        ...
        if onOff:
            ...
            G.microTransformScriptJobs.append(
                cmds.scriptJob(runOnce=False, killWithScene=False, event=('SelectionChanged', self.addMicroTransform)))
            G.microTransformScriptJobs.append(
                cmds.scriptJob(runOnce=False, killWithScene=False, event=('timeChanged', self.updateValues)))
            G.microTransformScriptJobs.append(
                cmds.scriptJob(runOnce=False, killWithScene=False, event=('Undo', self.updateValues)))
            G.microTransformScriptJobs.append(
                cmds.scriptJob(runOnce=False, killWithScene=False, event=('Redo', self.updateValues)))
            G.microTransformScriptJobs.append(
                cmds.scriptJob(runOnce=False, killWithScene=False, event=('DragRelease', self.release)))
    ...
    def changedMicroTransform(self, msg, mplug, otherMplug, clientData):
        ...
        if om.MNodeMessage.kAttributeSet == (
                om.MNodeMessage.kAttributeSet & msg) and not om.MGlobal.isUndoing() and not om.MGlobal.isRedoing():
    ...
    def addMicroTransform(self):
        ...
        for n, loopSel in enumerate(sel):
            ...
            G.microTransformIds.append(
                om.MNodeMessage.addAttributeChangedCallback(node, self.changedMicroTransform, clientData))
    ...
    def populateMenu(self, menu, *args):
        ...
        for loopOption in self.multiplierValues:
            ...
            cmds.menuItem(label=utilMod.toTitle(loopOption["name"]), radioButton=radioSelected,
                          command=lambda x, option=option, *args: self.setMultiplier(option), parent=menu)
