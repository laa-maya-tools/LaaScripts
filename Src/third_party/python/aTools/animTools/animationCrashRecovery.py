...
class AnimationCrashRecovery(object):
    ...
    def switch(self, onOff):
        ...
        if onOff:
            ...
            self.addMdgMessages()
            G.animationCrashRecoveryScriptJobs.append(
                cmds.scriptJob(runOnce=False, killWithScene=False, event=('SelectionChanged', self.addNodeMessages)))
    ...
    def setLed(self, state):
        ...
        if state == "on":
            ...
            cmds.image("animationCrashRecoveryLed", edit=True, image=uiMod.getImagePath("ACR_off"),
                       ann="Animation Crash Recovery is OFF")

        elif state == "blinking":
            self.blinkingLedState = not self.blinkingLedState
            image = "ACR_white_half" if self.blinkingLedState else "ACR_white_bright"
            cmds.image("animationCrashRecoveryLed", edit=True, image=uiMod.getImagePath(image),
                       ann="Animation Crash Recovery is saving animation")

        elif state == "blinking_red":
            self.blinkingLedState = not self.blinkingLedState
            image = "ACR_red_half" if self.blinkingLedState else "ACR_red_bright"
            cmds.image("animationCrashRecoveryLed", edit=True, image=uiMod.getImagePath(image),
                       ann="Animation Crash Recovery HIGHLY recommends you to save")

    def blinkingRed(self, onOff):

        if onOff:
            G.aToolsBar.timeoutInterval.setInterval(self.toggleRed, .3, id="ACR_red_blinking")
        else:
            G.aToolsBar.timeoutInterval.stopInterval("ACR_red_blinking")
    ...
    def optionBoxWindow(self, *args):
        ...
        def loadWindow():
            ...
            message = "%s\n%s\n\nWarning: Loading crash files after editing your Maya file can lead to unpredictable results." % (
            mayaFileName, time.ctime(modDate))
            ...
            buttonLoadSel = cmds.button(label='Load On Selection',
                                        command='cmds.layoutDialog(dismiss="load_selection")', w=110)
    ...
    def warningForLoading(self, message, height):
        ...
        def warningWindow():
            ...
            formLayout = cmds.setParent(query=True)
            icon = cmds.image(image=uiMod.getImagePath("ACR_white_bright"))
            titleText = cmds.text(label="You have newer animation. Do you want to load?", font="boldLabelFont",
                                  align="left")
            messageText = cmds.text(label=message, align="left")
            buttonLoad = cmds.button(label='Load', command='cmds.layoutDialog(dismiss="load")')
    ...
    def getSavedData(self, crashFolder=None, onlySelectedNodes=False, *args):
        ...
        data = self.getDataFromFiles("anim", folder, curveFiles, fileModTime, self.curveExt, progressInfo,
                                     onlySelectedNodes)
        ...
        animData = data[0]
        progressInfo = data[1]
        data = self.getDataFromFiles("attr", folder, attrFiles, fileModTime, self.attrExt, progressInfo,
                                     onlySelectedNodes)
        attrData = data[0]
    ...
    def applyAnimData(self, animData):
        ...
        animMod.applyAnimData(animData, pasteInPlace=False, showProgress=True,
                              status="aTools Animation Crash Recovery - Step 3/3 - Applying animation data...")
    ...
    def saveCurve(self, mayaFileName, curveStr):
        ...
        # save curve to disk
        aToolsMod.saveFileWithUser(
            "%s%s%s%s%s" % (self.baseFolderName, os.sep, sceneId, os.sep, self.baseLatestFolderName), curveStr,
            animData, ext=self.curveExt)
    ...
    def saveInfoData(self, mayaFileName, completed=False):
        ...
        aToolsMod.saveFileWithUser(
            "%s%s%s%s%s" % (self.baseFolderName, os.sep, sceneId, os.sep, self.baseLatestFolderName),
            self.infoDataFileName, infoData, ext="info")

    def loadInfoData(self):
        sceneId = aToolsMod.getSceneId()
        infoData = aToolsMod.loadFileWithUser(
            "%s%s%s%s%s" % (self.baseFolderName, os.sep, sceneId, os.sep, self.baseLatestFolderName),
            self.infoDataFileName, ext="info")
    ...
    def saveNonKeyedAttrs(self, mayaFileName, nonKeyedAttrsStr):
        ...
        # save curve to disk
        aToolsMod.saveFileWithUser(
            "%s%s%s%s%s" % (self.baseFolderName, os.sep, sceneId, os.sep, self.baseLatestFolderName), nonKeyedAttrsStr,
            attrData, ext=self.attrExt)
        self.saveInfoData(mayaFileName)
    ...
    def checkAndClearOldFiles(self):
        ...
        for loopIdFolder in allIdFolders:
            ...
            for loopInfoFile in [self.baseLatestFolderName, self.baseBackupFolderName]:
                infoDataFile = "%s%s%s%s%s%s%s.info" % (
                G.USER_FOLDER, os.sep, idFolder, os.sep, loopInfoFile, os.sep, self.infoDataFileName)
    ...
    def saveCrashLog(self, crashFilePath, beforeCrashPath, beforeCrashName):
        ...
        crashData = {"crashFilePath": crashFilePath, "beforeCrashPath": beforeCrashPath,
                     "beforeCrashName": beforeCrashName}
    ...
    def warnCrashLog(self, crashLog):
        ...
        message = "Looks like last Maya session crashed and saved a crash file.\n\nOriginal file:\n%s\n%s\n\nCrash saved file:\n%s\n%s\n\nDo you want to open the crash saved file?" % (
        beforeCrashName, beforeCrashModTime, crashFilePath, crashFileModTime)
        confirm = cmds.confirmDialog(title='aTools Animation Crash Recovery', message=message, button=['Yes', 'No'],
                                     defaultButton='Yes', cancelButton='No', dismissString='No')

        if confirm == 'Yes':
            if cmds.file(query=True, sceneName=True):
                message = "Save current file first? If you click NO, changes will be lost."
                confirm = cmds.confirmDialog(title='aTools Animation Crash Recovery', message=message,
    ...
    def addAnimSceneMessages(self):
        ...
        # SCENE MESSAGES
        G.ACR_messages["scene"].append(
            OpenMaya.MSceneMessage.addCallback(OpenMaya.MSceneMessage.kBeforeSave, self.beforeSave))
        G.ACR_messages["scene"].append(
            OpenMaya.MSceneMessage.addCallback(OpenMaya.MSceneMessage.kAfterSave, self.afterSave))
        G.ACR_messages["scene"].append(
            OpenMaya.MSceneMessage.addCallback(OpenMaya.MSceneMessage.kBeforeOpen, self.beforeOpen))
        G.ACR_messages["scene"].append(
            OpenMaya.MSceneMessage.addCallback(OpenMaya.MSceneMessage.kAfterOpen, self.afterOpen))
        G.ACR_messages["scene"].append(
            OpenMaya.MSceneMessage.addCallback(OpenMaya.MSceneMessage.kAfterNew, self.afterNew))
    ...
    def addNodeMessages(self):
        ...
        for n, loopSel in enumerate(currSel):
            ...
            G.ACR_messages["node"].append(
                OpenMaya.MNodeMessage.addAttributeChangedCallback(node, self.saveSelectedAttr, None))
