    ...
    def switch(self, onOff):
        ...
        if onOff:
            ...
            G.tumbleOnObjectsScriptJobs.append(
                cmds.scriptJob(runOnce=False, killWithScene=False, event=('DragRelease', self.update)))
            G.tumbleOnObjectsScriptJobs.append(
                cmds.scriptJob(runOnce=False, killWithScene=False, event=('SelectionChanged', self.update)))
            G.tumbleOnObjectsScriptJobs.append(
                cmds.scriptJob(runOnce=False, killWithScene=False, event=('timeChanged', self.update)))
    ...
    def update(self):
        ...
        if len(sel) > 0:
            ...
            if cmds.nodeType(sel) in allowedTypes:
                ...
                for loopCam in cams:
                    try:
                        cmds.setAttr("%s.tumblePivot" % loopCam, x, y, z)
                    except:
                        pass
