...
class ChangeBase():
    ...
    def undoIt(self):
        for i in range(len(self.changes), 0, -1):
            self.changes[i-1].undoIt()

    def redoIt(self):
        for change in self.changes:
            if hasattr(change, "redoIt"):
                change.redoIt()
            else:
                change.doIt()
    

class UndoOverwrite(ChangeBase):

    def commit(self):
        commit(undo=self.undoIt, redo=self.redoIt)


class AttributeChange(ChangeBase):

    class AttributeChangeRecord():
        def __init__(self, plug, oldValue, newValue):
            self.plug = plug
            self.oldValue = oldValue
            self.newValue = newValue

        def undoIt(self):
            self.plug.setDouble(self.oldValue)

        def redoIt(self):
            self.plug.setDouble(self.newValue)

    def performAndRecordChange(self, plug, value):
        change = AttributeChange.AttributeChangeRecord(plug, plug.asDouble(), value)
        change.redoIt()
