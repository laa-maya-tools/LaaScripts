...
class PerlinNoiseFactory(object):
    ...
    def _generate_gradient(self, points):
        ...
        if self.dimension == 1:
            # The first number is the slope at the point. The second one, an offset at that point.
            # This is done so the noise doesn't output 0 at integer numbers.
            r = random.Random(self.seed + points[0])
        ...
        for i in range(self.dimension):
            r = random.Random(self.seed + points[i])
            random_point.append((r.gauss(0, 1), r.gauss(0, 1)))
        # Then scale the result to a unit vector
        scale = sum(n * n for n, _ in random_point) ** -0.5
        return tuple((coord * scale, offset) for coord, offset in random_point)
    ...
    def get_plain_noise(self, *point):
        ...
        for grid_point in product(*grid_coords):
            ...
            dot = 0
            for i in range(self.dimension):
                dot += (gradient[i][0] * (point[i] - grid_point[i]) + gradient[i][1]) * 0.5
            dots.append(dot)
